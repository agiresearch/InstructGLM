...
class Arxiv_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random'): 
        ...
        self.mode = mode
        #self.prefix_1=......
        self.prefix='Categorize the article by topic: Node represents academic paper with a specific topic, link represents a citation between the two papers. Pay attention to the multi-hop link relationship between the nodes. '
        ...
        self.real_feature=load_pickle(os.path.join('Arxiv','my_data','Llama_real_feature.pkl')) #3
       ...
       # self.train_L2=load_pickle(os.path.join('Arxiv','my_data','L2.pkl'))  #5  看来L2也太大了,得现场生成了
        ...
        self.node_feature=load_pickle(os.path.join('Arxiv','my_data','Llama_node_feature.pkl'))  #8
        ...
        for xxxl in self.LA:
            xxx=xxx+xxxl+', '
        self.xxx='Classify the article according to its topic into one of the following categories:[{}].'.format(xxx)
        ...
        if task_name == 'link':
            ...
            if task_template['id'] == '1-1-1-1':    #记得都要加self.prefix,  记得做re_id: self.re_id
                if self.mode=='train': 
                   ...
                   # link_datum.append(point)
                    if rand_prob > 0.5:
                        ...
                        count=0
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>', node_list[:-2], '<extra_id_0>', '<extra_id_0>')
                ...
                if self.mode=='train': 
                    ...
                    if rand_prob > 0.5:
                        ...
                        middle_list=''
                        count=0
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>', node_list[:-2], middle_list[:-2],'<extra_id_0>', '<extra_id_0>','<extra_id_0>')
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(train_L2):  #count永远无法大于，最多等于
                            temp_text=source_text   #此时的temp_text是合格的
                ...
                if self.mode=='train': 
                    ...
                    middle_list=''
                    count=0
                    source_text =self.prefix + task_template['source'].format('<extra_id_0>', node_list[:-2],middle_list[:-2],'<extra_id_0>','<extra_id_0>')
                    go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                    already_idx=[]
                    while go_on and count < len(train_L2):  #count永远无法大于，最多等于
                        temp_text=source_text   
                ...
                if self.mode=='train': 
                    ...
                    if rand_prob > 0.5:
                        ...
                        middle_list=''
                        count=0
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>', node_list[:-2], middle_list[:-2],'<extra_id_0>', '<extra_id_0>','(<extra_id_0>,<extra_id_0>)')
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(train_L3):  #count永远无法大于，最多等于
                            temp_text=source_text   #此时的temp_text是合格的
            ...
            #
                ...
                if self.mode=='train': 
                   ...
                   # link_datum.append(point)
                    if rand_prob > 0.5:
                        ...
                        node_list=''    #这个在tokenizer看来不占位，非常好
                        count=0
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2], '<extra_id_0>',self.node_feature[link_datum[1]][0], '<extra_id_0>',self.node_feature[link_datum[0]][0])
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(self.train_L1[link_datum[0]]):  #count永远无法大于，最多等于
                            temp_text=source_text   #此时的temp_text是合格的
                        ...
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2],'<extra_id_0>',self.node_feature[negative][0], '<extra_id_0>',self.node_feature[link_datum[0]][0])
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(self.train_L1[link_datum[0]]):  #count永远无法大于，最多等于
                            temp_text=source_text   
                ...
                if self.mode=='train': 
                    ...
                    node_list=''    
                    count=0
                    source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2],'<extra_id_0>',self.node_feature[link_datum[0]][0])
                    go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                    already_idx=[]
                    while go_on and count < len(self.train_L1[link_datum[0]]):  #count永远无法大于，最多等于
                        temp_text=source_text   
                ...
                if self.mode=='train': 
                    ...
                    if rand_prob > 0.5:
                        ...
                        node_list=''    #这个在tokenizer看来不占位，非常好
                        count=0
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2], '<extra_id_0>',self.node_feature[link_datum[2]][0], '<extra_id_0>',self.node_feature[link_datum[0]][0])
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(train_L2):  #count永远无法大于，最多等于
                            temp_text=source_text   #此时的temp_text是合格的
                ...
                if self.mode=='train': 
                    ...
                    if rand_prob > 0.5:
                        ...
                        middle_list=''
                        count=0
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2], middle_list[:-2],'<extra_id_0>',self.node_feature[link_datum[2]][0], '<extra_id_0>',self.node_feature[link_datum[0]][0],'<extra_id_0>',self.node_feature[link_datum[1]][0])
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(train_L2):  #count永远无法大于，最多等于
                            temp_text=source_text   #此时的temp_text是合格的
                        ...
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2], middle_list[:-2],'<extra_id_0>',self.node_feature[negative][0], '<extra_id_0>',self.node_feature[link_datum[0]][0],'<extra_id_0>',self.node_feature[link_datum[1]][0])
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(train_L2):  #count永远无法大于，最多等于
                            temp_text=source_text   
                ...
                if self.mode=='train': 
                    ...
                    middle_list=''
                    count=0
                    source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2],middle_list[:-2],'<extra_id_0>',self.node_feature[link_datum[0]][0],'<extra_id_0>',self.node_feature[link_datum[1]][0])
                    go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                    already_idx=[]
                    while go_on and count < len(train_L2):  #count永远无法大于，最多等于
                        temp_text=source_text   
                ...
                if self.mode=='train': 
                    ...
                    if rand_prob > 0.5:
                        ...
                        node_list=''    #这个在tokenizer看来不占位，非常好
                        count=0
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2],'<extra_id_0>',self.node_feature[link_datum[3]][0],'<extra_id_0>',self.node_feature[link_datum[0]][0])
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(train_L3):  #count永远无法大于，最多等于
                            temp_text=source_text   #此时的temp_text是合格的
                ...
                if self.mode=='train': 
                    ...
                    if rand_prob > 0.5:
                        ...
                        middle_list=''
                        count=0
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2], middle_list[:-2],'<extra_id_0>',self.node_feature[link_datum[3]][0], '<extra_id_0>',self.node_feature[link_datum[0]][0],'<extra_id_0>',self.node_feature[link_datum[1]][0],'<extra_id_0>',self.node_feature[link_datum[2]][0])
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(train_L3):  #count永远无法大于，最多等于
                            temp_text=source_text   #此时的temp_text是合格的
                        ...
                        source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2], middle_list[:-2],'<extra_id_0>',self.node_feature[negative][0], '<extra_id_0>',self.node_feature[link_datum[0]][0],'<extra_id_0>',self.node_feature[link_datum[1]][0],'<extra_id_0>',self.node_feature[link_datum[2]][0])
                        go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                        already_idx=[]
                        while go_on and count < len(train_L3):  #count永远无法大于，最多等于
                            temp_text=source_text   
                ...
                if self.mode=='train': 
                    ...
                    middle_list=''
                    count=0
                    source_text =self.prefix + task_template['source'].format('<extra_id_0>',self.node_feature[link_datum[0]][0], node_list[:-2],middle_list[:-2],'<extra_id_0>',self.node_feature[link_datum[0]][0],'<extra_id_0>',self.node_feature[link_datum[1]][0],'<extra_id_0>',self.node_feature[link_datum[2]][0])
                    go_on=True if len(self.tokenizer.tokenize(source_text))+1 < self.l_max else False
                    already_idx=[]
                    while go_on and count < len(train_L3):  #count永远无法大于，最多等于
                        temp_text=source_text   
...
#
                ...
                abs=self.node_feature[point][1] 
                if len(self.tokenizer.encode(abs))>330:
                    abs=self.tokenizer.decode(self.tokenizer.encode(abs)[:330],skip_special_tokens=True)
