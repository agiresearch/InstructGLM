...
class FP(nn.Module):
    def __init__(self,llama_embed,real):
       ...
       # self.trans_3=nn.Linear(128,4096,bias=False)
    ...
    def forward(self, input_ids):
        transfered=self.trans_2(self.rac(self.sln(self.trans_1(self.real_feature)))) #+ self.trans_3(self.real_feature)
...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True,val_list=None):
        ...
        if train:  #若测试,则全部放到test里初始model, per checkpoint地初始
            ...
            if re_start==1:
                ...
                if self.verbose:
                    self.model.save_pretrained("C_restart")

            if re_start==2:
                from peft import PeftModel, PeftConfig
                peft_model_id ='./C_restart'
  ...
  #              self.model.gradient_checkpointing_enable()
    ...
    # cast all non INT8 parameters to fp32
      ...
      #  self.model.resize_token_embeddings(self.tokenizer.vocab_size+1+169343) #若llama加link,则必须用
        ...
        if train and re_start==1:
            ...
            ckpt_path="first_s_1_0.0001_8_Arxiv_end.pth"
        ...
        if train and re_start==2:
            ...
            self.first_model.sln.bias.data=state_dict['sln.bias']
            self.first_model.sln.weight.data=state_dict['sln.weight']
   ...
   #         print(self.first_model.trans_1.weight)
        ...
        if train and re_start==3:   ###use this 
            self.first_model.sln.bias.data=load_pickle('./C_first_restart/sln_bias.pkl')
            self.first_model.sln.weight.data=load_pickle('./C_first_restart/sln_weight.pkl')
        #    self.first_model.sln2.weight.data=load_pickle('./first_restart/sln2_weight.pkl')
         #   self.first_model.sln2.bias.data=load_pickle('./first_restart/sln2_bias.pkl')
            self.first_model.trans_1.weight.data=load_pickle('./C_first_restart/trans_1_weight.pkl')
                                                               #         print(self.first_model.trans_1.weight)
            self.first_model.trans_2.weight.data=load_pickle('./C_first_restart/trans_2_weight.pkl')
          #  self.first_model.trans_3.weight.data=load_pickle('./first_restart/trans_3_weight.pkl')
        #    ddd=[]
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            for step_i, batch in enumerate(self.train_loader):   #step_i可用来操作acc_step\
                ...
                if global_step==len(self.train_loader)//8:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"Cora_first_s_{}_{}_8_{}_{}_mmid1.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)//4:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"Cora_first_s_{}_{}_8_{}_{}_mid1.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)*3//8:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"Cora_first_s_{}_{}_8_{}_{}_mmid2.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)//2:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"Cora_first_s_{}_{}_8_{}_{}_mid2.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)*5//8:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"Cora_first_s_{}_{}_8_{}_{}_mmid3.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)*3//4:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"Cora_first_s_{}_{}_8_{}_{}_mid3.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)*7//8:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"Cora_first_s_{}_{}_8_{}_{}_mend.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
            ...
            if self.verbose:
                ...
                torch.save(self.first_model.state_dict(),"Cora_first_s_{}_{}_8_{}_{}_end.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
            dist.barrier()
 ...
 ##               dist.barrier()
   ...
   ##             valid_results = new_reduce_dict(valid_results)   #
    ...
    def test(self):   #没有封装DDP
        for epoch in range(16,8*self.args.epoch):
            #load 对应模型
            #ckpt_path = "{}_{}_{}.pth".format(epoch+1,self.args.lr,self.args.world_size)  #GPU数目注意一下
            if (epoch+1)%8==1:
                doc_prefix='./llama_{}_mmid1/'.format(epoch//8+1)
                ckpt_path = "Cora_first_s_{}_{}_8_{}_{}_mmid1.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps) 
            elif (epoch+1)%8==2:
                doc_prefix='./llama_{}_mid1/'.format(epoch//8+1)
                ckpt_path = "Cora_first_s_{}_{}_8_{}_{}_mid1.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps) 
            elif (epoch+1)%8==3:
                doc_prefix='./llama_{}_mmid2/'.format(epoch//8+1)
                ckpt_path = "Cora_first_s_{}_{}_8_{}_{}_mmid2.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps) 
            elif (epoch+1)%8==4:
                doc_prefix='./llama_{}_mid2/'.format(epoch//8+1)
                ckpt_path = "Cora_first_s_{}_{}_8_{}_{}_mid2.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
            elif (epoch+1)%8==5:
                doc_prefix='./llama_{}_mmid3/'.format(epoch//8+1)
                ckpt_path = "Cora_first_s_{}_{}_8_{}_{}_mmid3.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
            elif (epoch+1)%8==6:
                doc_prefix='./llama_{}_mid3/'.format(epoch//8+1)
                ckpt_path = "Cora_first_s_{}_{}_8_{}_{}_mid3.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
            elif (epoch+1)%8==7:
                doc_prefix='./llama_{}_mend/'.format(epoch//8+1)
                ckpt_path = "Cora_first_s_{}_{}_8_{}_{}_mend.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
            else:
                doc_prefix='./llama_{}_end/'.format(epoch//8+1)
                ckpt_path = "Cora_first_s_{}_{}_8_{}_{}_end.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
        #    ckpt_path='sim_'+ckpt_path
         #   doc_prefix=doc_prefix+'sim_'
...
#load model的程序在这里改:
            ...
            dist.barrier()
                ...
                #per-epoch地将测试结果写入一个txt文件
            if self.verbose:
                acc_file=open('ReCora_Llama_s_{}_{}.txt'.format(self.args.lr,self.args.gradient_accumulation_steps),'a')                           #文件名字注意一下
                if (epoch+1)%8==1:
                     ...
                     #   print(results)
                      #  print()
                       # print(batch['target_text'])
                        #print(self.val_loader.dataset.len_transductive)
               #     print(len(results))
...
def main_worker(gpu, args):     # the gpu is the args.local_rank
    ...
    if args.inference:
        ...
        if args.valid == 'Cora':
            ...
            val_task_list = {
            #'classification':[['5-5-5-5','6-6-6-6'],['2-1-1-1','2-1-1-2','2-3-1-1','2-3-1-2'],['2-1-2-1','2-1-2-2','2-1-2-3','2-1-2-4','2-3-2-1','2-3-2-2','2-3-2-3','2-3-2-4'],['2-1-3-1','2-1-3-2','2-1-3-3','2-1-3-4','2-3-3-1','2-3-3-2','2-3-3-3','2-3-3-4']]
            'classification':[['6-6-6-6','6-6-6-7']]#,['2-1-2-2','2-1-2-4','2-3-2-2','2-3-2-4'],['2-1-3-2','2-1-3-4','2-3-3-2','2-3-3-4']]
            }
        else:
            val_task_list = {
            'link':[['1-1-1-1','1-1-1-2','1-2-1-1','1-2-1-2'],['1-1-2-1','1-1-2-2','1-1-2-3','1-1-2-4','1-2-2-1','1-2-2-2','1-2-2-3','1-2-2-4'],['1-1-3-1','1-1-3-2','1-1-3-3','1-1-3-4','1-2-3-1','1-2-3-2','1-2-3-3','1-2-3-4']],
