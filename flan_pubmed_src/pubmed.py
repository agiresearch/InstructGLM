...
class Arxiv_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random'): 
        ...
        self.re_id=load_pickle(os.path.join('PubMed','my_data','final_pub_re_id.pkl'))  #2
        self.l_max=self.args.max_text_length
        self.real_feature=load_pickle(os.path.join('PubMed','my_data','final_norm_pub_real_feature.pkl')) #3
        self.train_L1=load_pickle(os.path.join('PubMed','my_data','final_pub_L1.pkl'))  #4

        self.transductive=load_pickle(os.path.join('PubMed','my_data','final_pub_transductive.pkl'))  #6 a list
        self.classification=load_pickle(os.path.join('PubMed','my_data','final_pub_classification.pkl'))  #7
        self.node_feature=load_pickle(os.path.join('PubMed','my_data','final_pubmed_node_feature.pkl'))  #8
    ...
    def compute_datum_info_train(self):
        ...
        for key in list(self.task_list.keys()):     
            if key == 'link':
                ...
                for tems in self.task_list[key]:
                    if '2-3-1-2' in tems:
                        self.total_length += len(self.classification) * 20   #90941 nodes for training
                        for i in tqdm(range(self.total_length - curr)):
                            self.datum_info.append((i + curr, key, i // 20,tems[i%2],'transductive')) 
                            #self.datum_info.append((i + curr, key, i // 4,tems[i % 4],'transductive'))  
                        curr = self.total_length
                    elif '2-1-2-2' in tems:

                        self.total_length += len(self.classification) * 20
                        for i in tqdm(range(self.total_length - curr)):
                            self.datum_info.append((i + curr, key, i // 20,tems[i%4],'transductive'))
                        ...
                        self.total_length += len(self.classification) * 20
                        for i in tqdm(range(self.total_length - curr)):
                            self.datum_info.append((i + curr, key, i // 20,'5-6','transductive'))
