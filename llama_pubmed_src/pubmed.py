...
class Arxiv_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random'): 
        ...
        self.label_map=load_pickle(os.path.join('PubMed','my_data','final_pub_label_map.pkl'))  #1
        self.re_id=load_pickle(os.path.join('PubMed','my_data','Llama_final_pub_re_id.pkl'))  #2   change
        self.llama_embed=load_pickle(os.path.join('PubMed','my_data','Llama_embeds.pkl'))  #add.
        self.l_max=612#self.args.max_text_length + 60
        self.real_feature=load_pickle(os.path.join('PubMed','my_data','Llama_final_norm_pub_real_feature.pkl')) #3 change
        self.train_L1=load_pickle(os.path.join('PubMed','my_data','final_pub_L1.pkl'))  #4
        self.transductive=load_pickle(os.path.join('PubMed','my_data','big_final_pub_transductive.pkl'))  #6 a list
        self.classification=load_pickle(os.path.join('PubMed','my_data','big_final_pub_classification.pkl'))  #7
        self.node_feature=load_pickle(os.path.join('PubMed','my_data','full_final_pubmed_node_feature.pkl'))  #8
...
#
                ...
                while len(self.tokenizer.encode(tit))>60:
                    tit=self.tokenizer.decode(self.tokenizer.encode(tit)[:59],skip_special_tokens=True)
             #   source_text =task_template['source'].format('<extra_id_0>',tit, abss, '<extra_id_0>',tit)
                while len(self.tokenizer.encode(abss))>900:
                    abss=self.tokenizer.decode(self.tokenizer.encode(abss)[:899],skip_special_tokens=True)
                source_text =task_template['source'].format('<extra_id_0>',tit, abss, '<extra_id_0>',tit)
