...
class FP(nn.Module):
    def __init__(self,llama_embed,real):
       ...
       # self.trans_3=nn.Linear(128,4096,bias=False)
        ...
        self.real_feature=real.to(torch.float32)#.to(self.trans_1.weight.dtype)
        #self.real_feature.to(torch.float32)
        #
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            for step_i, batch in enumerate(self.train_loader):   #step_i可用来操作acc_step\
                ...
                if global_step==len(self.train_loader)//8:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"pub_first_s_{}_{}_8_{}_{}_mmid1.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)//4:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"pub_first_s_{}_{}_8_{}_{}_mid1.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)*3//8:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"pub_first_s_{}_{}_8_{}_{}_mmid2.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)//2:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"pub_first_s_{}_{}_8_{}_{}_mid2.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)*5//8:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"pub_first_s_{}_{}_8_{}_{}_mmid3.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)*3//4:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"pub_first_s_{}_{}_8_{}_{}_mid3.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
                if global_step==len(self.train_loader)*7//8:
                    ...
                    if self.verbose:
                        ...
                        torch.save(self.first_model.state_dict(),"pub_first_s_{}_{}_8_{}_{}_mend.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
            ...
            if self.verbose:
                ...
                torch.save(self.first_model.state_dict(),"pub_first_s_{}_{}_8_{}_{}_end.pth".format(epoch+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps))
            dist.barrier()
 ...
 ##               dist.barrier()
   ...
   ##             valid_results = new_reduce_dict(valid_results)   #
    ...
    def test(self):   #没有封装DDP
        for epoch in range(6,8*self.args.epoch):
            #load 对应模型
            #ckpt_path = "{}_{}_{}.pth".format(epoch+1,self.args.lr,self.args.world_size)  #GPU数目注意一下
            if (epoch+1)%8==1:
                doc_prefix='./llama_{}_mmid1/'.format(epoch//8+1)
                ckpt_path = "pub_first_s_{}_{}_8_{}_{}_mmid1.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps) 
            elif (epoch+1)%8==2:
                doc_prefix='./llama_{}_mid1/'.format(epoch//8+1)
                ckpt_path = "pub_first_s_{}_{}_8_{}_{}_mid1.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps) 
            elif (epoch+1)%8==3:
                doc_prefix='./llama_{}_mmid2/'.format(epoch//8+1)
                ckpt_path = "pub_first_s_{}_{}_8_{}_{}_mmid2.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps) 
            elif (epoch+1)%8==4:
                doc_prefix='./llama_{}_mid2/'.format(epoch//8+1)
                ckpt_path = "pub_first_s_{}_{}_8_{}_{}_mid2.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
            elif (epoch+1)%8==5:
                doc_prefix='./llama_{}_mmid3/'.format(epoch//8+1)
                ckpt_path = "pub_first_s_{}_{}_8_{}_{}_mmid3.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
            elif (epoch+1)%8==6:
                doc_prefix='./llama_{}_mid3/'.format(epoch//8+1)
                ckpt_path = "pub_first_s_{}_{}_8_{}_{}_mid3.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
            elif (epoch+1)%8==7:
                doc_prefix='./llama_{}_mend/'.format(epoch//8+1)
                ckpt_path = "pub_first_s_{}_{}_8_{}_{}_mend.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
            else:
                doc_prefix='./llama_{}_end/'.format(epoch//8+1)
                ckpt_path = "pub_first_s_{}_{}_8_{}_{}_end.pth".format(epoch//8+1,self.args.lr,self.args.train,self.args.gradient_accumulation_steps)
...
#load model的程序在这里改:
            ...
            if self.verbose:
                ...
                for kk in valid_results.keys():#
                    if kk.startswith('1'):
                        ...
                        if self.args.train=='Cora' or self.args.train=='Arxiv' or self.args.train=='PubMed':
            ...
            dist.barrier()
                ...
                #per-epoch地将测试结果写入一个txt文件
            if self.verbose:
                acc_file=open('Pub_Llama_s_{}_{}.txt'.format(self.args.lr,self.args.gradient_accumulation_steps),'a')                           #文件名字注意一下
                if (epoch+1)%8==1:
...
def main_worker(gpu, args):     # the gpu is the args.local_rank
    ...
    if not args.inference:
        ...
        if args.train == 'Cora' or args.train=='PubMed':
            train_task_list = {
