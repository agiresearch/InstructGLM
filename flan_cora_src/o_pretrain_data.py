...
class P5_Amazon_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random'): 
        ...
        self.mode = mode
        if self.mode == 'train':    #  review 和 exp是分开的？
            ...
            if self.rating_augment:
                ...
                self.rating_data = self.review_data
        elif self.mode == 'test':   #在pretrain.py里面没有用到这一部分
...
#        mmmm=-1
        for line in self.sequential_data:    #这里都是id数字化了, 但是是数字字符串
 ...
 #           if len(items) > mmmm:
  #              mmmm=len(items)
            ...
            for item in items:
                item_count[item] += 1       #这个存储了每个item在dataset中出现了多少次
        
   #     print()
    #    print("mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm")
     #   print(mmmm)     longest: 204 (for sequential_items list)
      #  print()
                
        self.all_item = list(item_count.keys())
        
        #print(self.all_item)     12101
        #print(list(user_item.keys()))   22363,所以所有user对应的node_id应该从12102开始，全部加上12101
        ...
        if self.mode == 'test':     #仅在测试的时候考虑负样本
        ...
        self.user2id = datamaps['user2id']     #相比之下我只有node_id
        ...
        for meta in parse(os.path.join('data', split, 'meta.json.gz')):
            self.meta_data.append(meta)                      #为何要把这个搞出来？，这里可没有删除原文件里任何信息，   没问题
        self.meta_dict = {}

        for i, meta_item in enumerate(self.meta_data):
            self.meta_dict[meta_item['asin']] = i      #为何要把这个搞出来？，    没事，搞出来是为了索取其他元信息，不删不碍事，访问不到
    ...
    def compute_datum_info(self):
        curr = 0
        for key in list(self.task_list.keys()):     #我必须把这里所有都打成4维度的
            if key == 'rating':
                ...
                self.total_length += len(self.sequential_data) * self.sample_numbers[key]  #暂时是借了sequential_data
                for i in range(self.total_length - curr):
                    self.datum_info.append((i + curr, key, i // self.sample_numbers[key]))
                curr = self.total_length
    ...
    def __getitem__(self, idx):
        ...
        if len(datum_info_idx) == 3:
            ...
            datum_idx = datum_info_idx[2]   #important
        ...
        if task_name == 'rating':            
            ...
            if task_template['id'] == '1-1':   # 值得注意 use2id, item2id
                ...
                target_text = task_template['target'].format(self.gaussian_sampling(rating_datum))
            elif task_template['id'] == '1-3':      #这个值得借鉴
                ...
                if 'reviewerName' in rating_datum:
                    ...
                    user_desc = rating_datum['reviewerID']
                if 'title' in self.meta_data[self.meta_dict[rating_datum['asin']]]:   #本质上是因为当年漏扒了title信息
                    title = self.meta_data[self.meta_dict[rating_datum['asin']]]['title']
                else:
                    title = 'unknown title'
                ...
                if 'reviewerName' in rating_datum:
                    ...
                    user_desc = rating_datum['reviewerID']
                if 'title' in self.meta_data[self.meta_dict[rating_datum['asin']]]:   #这个判断条件写得值得关注
                    title = self.meta_data[self.meta_dict[rating_datum['asin']]]['title']
                else:
                    title = 'unknown title'
                source_text = task_template['source'].format(user_desc, title)
            ...
            user_id = sequence[0]    #字符串

            user_desc = self.user_id2name[user_id]     # this is used for user_description，desc is just name???
            ...
            task_candidates = self.task_list[task_name]    #为什么不”遍历“一遍呢
            task_idx = random.randint(0, len(task_candidates)-1) # random choose the task index for task_candidates
            task_template = self.all_tasks['sequential'][task_candidates[task_idx]]
            assert task_template['task'] == 'sequential'
            
            if task_template['id'] == '2-1':   #ok，我想清楚了， sequential在我的project里面可以做
                rand_prob = random.random()
                if rand_prob > 0.5:
            ...
            elif task_template['id'] == '2-11':       #'2-11'值得借鉴
            ...
            if task_template['id'] == '5-1':
                ...
                if rand_prob > 0.5:
                    ...
                    user_seq = self.user_items[user_id]   # user_seq的用处
                    candidate_samples = []
                    candidate_num = 1
                    while len(candidate_samples) < candidate_num:
                        if self.sample_type == 'random':
                ...
                if rand_prob > 0.5:
                    ...
                    while len(candidate_samples) < candidate_num:
                        ...
                        candidate_samples.extend(sample_ids)
                    candidate_samples = candidate_samples[:candidate_num]
                    if 'title' in self.meta_data[self.meta_dict[self.id2item[candidate_samples[0]]]]:  #id2item
                        title = self.meta_data[self.meta_dict[self.id2item[candidate_samples[0]]]]['title']
                    else:
                        title = 'unknown title'
                    source_text = task_template['source'].format(user_id, title)
                ...
                candidate_samples = candidate_samples[:candidate_num]
                candidate_samples.extend([target_item])
                random.shuffle(candidate_samples)   #如果是构建candidate set记得shuffle一下，这里做的好
...
#self_dataset
class new_P5_Amazon_Dataset(P5_Amazon_Dataset): 
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random' ):
        super().__init__(all_tasks, task_list, tokenizer, args, sample_numbers, mode, split, rating_augment, sample_type)
    ...
    def __getitem__(self, idx):
        ...
        datum_info_idx = self.datum_info[idx]       #datum_info在__init__里面计算好了
        assert datum_info_idx[0] == idx
        if len(datum_info_idx) == 3:
            task_name = datum_info_idx[1]
            datum_idx = datum_info_idx[2]
        ...
        if task_name == 'link':  
            ...
            if self.mode == 'train':     # To get purchase_history & target_item
                end_candidates = [_ for _ in range(max(2, len(sequence) - 6), len(sequence) - 3)]
                end_index = random.randint(0, len(end_candidates)-1)
                end_pos = end_candidates[end_index]    #最小最小也是2

                start_candidates = [_ for _ in range(1, min(4, end_pos))]
                start_index = random.randint(0, len(start_candidates)-1)
                start_pos = start_candidates[start_index]   #最小是1， 最大是end_pos-1

                #purchase_history 中元素也是字符串
                purchase_history = sequence[start_pos:end_pos+1] # sample a history sequence from the full user purchase history
                #

                #不知道序列最大可接受长度够不够    we would better do not do this 
                #for every in range(len(purchase_history)):
                 #   purchase_history[every]='node_'+purchase_history[every]

                target_item = sequence[end_pos+1]  #最小最小也是3
            ...
            task_candidates = self.task_list[task_name]
            task_idx = random.randint(0, len(task_candidates)-1) # random choose the task index for task_candidates
            task_template = self.all_tasks['link'][task_candidates[task_idx]]
            assert task_template['task'] == 'link'
            
            if task_template['id'] == '6-1':
               #user_id --> str(int(user_id)+12101)
                source_text = task_template['source'].format(str(int(user_id)+12101), ' , '.join(purchase_history), str(int(user_id)+12101))
                target_text = task_template['target'].format(target_item)
            ...
            elif task_template['id'] == '6-3':       #'2-11'值得借鉴
                symbol_prob = random.random()
                if symbol_prob > 0.5:
                    symbol = ' , '
                else:
                ...
                rand_prob = random.random()
                if rand_prob > 0.5:
                    source_text = task_template['source'].format(str(int(user_id)+12101), symbol.join(purchase_history), target_item, str(int(user_id)+12101))
                    target_text = task_template['target'].format('yes')
                    ...
                    while len(candidate_samples) < candidate_num:
                        ...
                        candidate_samples.extend(sample_ids)
                    candidate_samples = candidate_samples[:candidate_num]
                    source_text = task_template['source'].format(str(int(user_id)+12101), symbol.join(purchase_history), candidate_samples[0], str(int(user_id)+12101))
                    target_text = task_template['target'].format('no')



            elif task_template['id'] == '6-4':       #'2-11'值得借鉴
                symbol_prob = random.random()
                if symbol_prob > 0.5:
                    symbol = ' , '
                else:
        ...
        ##print('sourceseeeeeeeeeeeeeeeee')
        ##print(source_text)
        ...
        out_dict['loss_weight'] = loss_weight
        out_dict['temp_id'] = task_template['id']   #我重写了__get_itme__
