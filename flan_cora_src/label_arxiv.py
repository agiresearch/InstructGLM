...
class label_Arxiv_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random'): 
        ...
        self.label_map=load_pickle(os.path.join('Cora','my_data','final_cora_label_map.pkl'))  #1
        self.re_id=load_pickle(os.path.join('Cora','my_data','final_cora_re_id.pkl'))  #2
        self.l_max=self.args.max_text_length
        self.real_feature=load_pickle(os.path.join('Cora','my_data','final_cora_real_feature.pkl')) #3
        self.train_L1=load_pickle(os.path.join('Cora','my_data','final_cora_L1.pkl'))  #4
       # self.train_L2=load_pickle(os.path.join('Arxiv','my_data','L2.pkl'))  #5  看来L2也太大了,得现场生成了
        self.transductive=load_pickle(os.path.join('Cora','my_data','big_final_cora_transductive.pkl'))  #6 a list
        self.classification=load_pickle(os.path.join('Cora','my_data','big_final_cora_classification.pkl'))  #7
        self.node_feature=load_pickle(os.path.join('Cora','my_data','final_cora_node_feature.pkl'))  #8
    ...
    def compute_datum_info_train(self):
        ...
        for key in list(self.task_list.keys()):     
            if key == 'link':
                ...
                for tems in self.task_list[key]:
                    if '2-1-1-2' in tems:
                        self.total_length += len(self.classification) * 4   #90941 nodes for training
...
#
        if task_name == 'link':
            ...
            elif task_template['id'] == '1-7-7-4':   #记得都要加self.prefix,  记得做re_id: self.re_id
            ...
            elif task_template['id'] == '1-7-7-3':    #记得都要加self.prefix,  记得做re_id: self.re_id
                link_abs=self.node_feature[link_datum[0]][1] 
                if self.mode=='train': 
                    real_id=[self.re_id[link_datum[0]]]
                    rand_prob = random.random()
                ...
                if self.mode=='train': 
                    ...
                    for ele in train_L2:#[:200]:
                     #   ta=copy.deepcopy(self.train_L1[ele[1]])
                      #  random.shuffle(ta)
                        for el in self.train_L1[ele[1]]:#ta[:30]:
                            if el!=link_datum[0]:
...
#
                ...
                if self.mode!=None: 
                    ...
                    while go_on and count < len(self.train_L1[point]):  
                        ...
                        idx=int(np.random.choice(select,1,replace=False)[0])
                        already_idx.append(idx)
                        if self.train_L1[point][idx] in self.classification and self.train_L1[point][idx]!=point:
                            node_list=node_list+'(<extra_id_0>,{}), '.format(self.label_map[self.train_L1[point][idx]]+','+self.node_feature[self.train_L1[point][idx]][0])
                        else:
                            node_list=node_list+'(<extra_id_0>,{}), '.format(self.node_feature[self.train_L1[point][idx]][0])
                        real_id.append(self.re_id[self.train_L1[point][idx]])
                        #node_list=node_list+'{}, '.format(self.re_id[self.train_L1[point][idx]])
                ...
                if self.mode!=None: 
                    ...
                    while go_on and count < len(train_L2):  #count永远无法大于，最多等于
                        ...
                        idx=int(np.random.choice(select,1,replace=False)[0])
                        already_idx.append(idx)
                        if train_L2[idx][1] in self.classification and train_L2[idx][1]!=point:
                            node_list=node_list+'(<extra_id_0>,{}), '.format(self.label_map[train_L2[idx][1]]+','+self.node_feature[train_L2[idx][1]][0])
                        else:
                            node_list=node_list+'(<extra_id_0>,{}), '.format(self.node_feature[train_L2[idx][1]][0])
                        real_id.append(self.re_id[train_L2[idx][1]])
                        #node_list=node_list+'{}, '.format(self.re_id[self.train_L2[point][idx][1]])
                ...
                if self.mode!=None: 
                    ...
                    while go_on and count < len(train_L2):  #count永远无法大于，最多等于
                        ...
                        id_1.append(self.re_id[train_L2[idx][1]])
                        #node_list=node_list+'{}, '.format(self.re_id[self.train_L2[point][idx][1]])
                        if train_L2[idx][0] in self.classification and train_L2[idx][0]!=point:
                            middle_list=middle_list+'(<extra_id_0>,{}), '.format(self.label_map[train_L2[idx][0]]+','+self.node_feature[train_L2[idx][0]][0])
                        else:
                            middle_list=middle_list+'(<extra_id_0>,{}), '.format(self.node_feature[train_L2[idx][0]][0])
                        id_2.append(self.re_id[train_L2[idx][0]])
                        #middle_list=middle_list+'{}, '.format(self.re_id[self.train_L2[point][idx][0]])
                  ...
                  #      ta=copy.deepcopy(self.train_L1[ele[1]])
                   ...
                   # train_L3=[]   #这个用作node采样
                    ...
                    while go_on and count < len(train_L3):  #count永远无法大于，最多等于
                        ...
                        idx=int(np.random.choice(select,1,replace=False)[0])
                        already_idx.append(idx)
                        if train_L3[idx][2] in self.classification and train_L3[idx][2]!=point:
                            node_list=node_list+'(<extra_id_0>,{}), '.format(self.label_map[train_L3[idx][2]]+','+self.node_feature[train_L3[idx][2]][0])
                        else:
                            node_list=node_list+'(<extra_id_0>,{}), '.format(self.node_feature[train_L3[idx][2]][0])
                        real_id.append(self.re_id[train_L3[idx][2]])
                        #node_list=node_list+'{}, '.format(self.re_id[train_L3[idx][2]])
              ...
              #          ta=copy.deepcopy(self.train_L1[ele[1]])
               #         random.shuffle(ta)
                    ...
                    while go_on and count < len(train_L3):  #count永远无法大于，最多等于
                        ...
                        label_left,label_right=False,False
                        if train_L3[idx][0] in self.classification and train_L3[idx][0]!=point:
                            label_left=True
                        if train_L3[idx][1] in self.classification and train_L3[idx][1]!=point:
                            label_right=True

                        if label_left and label_right:
                            middle_list=middle_list+'(<extra_id_0>,{}; <extra_id_0>,{}), '.format(self.label_map[train_L3[idx][0]]+','+self.node_feature[train_L3[idx][0]][0],self.label_map[train_L3[idx][1]]+','+self.node_feature[train_L3[idx][1]][0])
                        elif label_left and not label_right:
                            middle_list=middle_list+'(<extra_id_0>,{}; <extra_id_0>,{}), '.format(self.label_map[train_L3[idx][0]]+','+self.node_feature[train_L3[idx][0]][0],self.node_feature[train_L3[idx][1]][0])
                        elif not label_left and label_right:
                            middle_list=middle_list+'(<extra_id_0>,{}; <extra_id_0>,{}), '.format(self.node_feature[train_L3[idx][0]][0],self.label_map[train_L3[idx][1]]+','+self.node_feature[train_L3[idx][1]][0])
                        else:
                            middle_list=middle_list+'(<extra_id_0>,{}; <extra_id_0>,{}), '.format(self.node_feature[train_L3[idx][0]][0],self.node_feature[train_L3[idx][1]][0])
                        id_2.append(self.re_id[train_L3[idx][0]])
                        id_2.append(self.re_id[train_L3[idx][1]])
