...
class Arxiv_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='', rating_augment=False, sample_type='random'): 
        ...
        self.re_id=load_pickle(os.path.join('Arxiv','data','Llama_re_id.pkl'))  #2 Map node IDs to new index IDs in the extended LLM vocabulary.
        self.llama_embed=load_pickle(os.path.join('Arxiv','data','Llama_embeds.pkl')) #3 The words embedding of Llama-7b, which is freezed during LoRA Tuning. 
        self.l_max=2048 # It can be adjusted if CUDA Out of Memory
        self.real_feature=load_pickle(os.path.join('Arxiv','data','Llama_giant.pkl')) #4 Preprocessed Numerical Node Feature Embedding
        ...
        self.transductive=load_pickle(os.path.join('Arxiv','data','transductive.pkl'))  #6 a list, store test (transductive.pkl)/ val (val.pkl) node ID
        self.classification=load_pickle(os.path.join('Arxiv','data','classification.pkl'))  #7 store train node ID
        self.node_feature=load_pickle(os.path.join('Arxiv','data','full_Llama_node_feature.pkl'))  #8 store nodes' raw text feature(e.g. title/ abstract)
